...
(function($){
	...
	var api = wipLayoutManager = {
		...
		options : {
			speedOnMove : 250,
			ajaxThemeAction : 'wipProcessPageLayoutAjax'
		},
		...
		attachLayoutEditListeners : function() {
			var that = this;
			$('#webinpixels_wraper').bind('click', function(e) {
		...
		detectPossibleParentID : function(){
		
			var parentIDs = $('#webinpixels_wraper').find('input[name^="wip_page_parent_id"]');
			
			if( parentIDs.length ){
				var inpVal = parentIDs.map(function() {
								return $(this).val();
							}).get();
				var nextID = Math.max.apply( Math, inpVal ) + 1;
		...
		detectPossibleContentID : function(){
			
			var contentIDs = $('#webinpixels_wraper').find('input[name^="id"]');
			if( contentIDs.length ){
				var inpVal = contentIDs.map(function() {
								return $(this).val();
							}).get();
				var nextID = Math.max.apply( Math, inpVal ) + 1;
		...
		addItemToLayout : function(){
			...
			$('.col_insert').each( function(){
				...
				anchor.bind('click', function(e){
					...
					var possibleID = that.detectPossibleParentID;
										...
										t.sortable({
											...
											update: function(event, ui) {
												...
												if( parentNow.hasClass('wip-layout-item') ){
													var inputParentVal = parentNow.find('input[name^="wip_page_parent_id"]').val(),
		...
		eventOnAddContent : function(){
			var selectPr, thisID, 
				that = this;
			$('#webinpixels_wraper').bind('change', function(e) {
						...
						if( -1 != e.target.parentNode.className.indexOf('module-for-column') ){
							...
							if( 0 != e.target.value ){
								...
								var tgPar = e.target.parentNode,
									...
									possibleID = that.detectPossibleContentID;
