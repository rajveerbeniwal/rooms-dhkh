...
// class publiciable emulation
// cf. http://www.webkreator.com/php/techniques/php-static-class-publiciables.html
...
class nusoap_base {
	...
	*/
	function serialize_val($val,$name=false,$type=false,$name_ns=false,$type_ns=false,$attributes=false,$use='encoded',$soapval=false) {
		...
		$this->appendDebug('value=' . $this->publicDump($val));
		$this->appendDebug('attributes=' . $this->publicDump($attributes));
		...
		switch(true) {
			...
			case is_object($val):
		    	...
		    	if (get_class($val) == 'soapval') {
					...
					foreach(get_object_publics($val) as $k => $v){
    ...
    */
	...
	$this->appendDebug($this->publicDump($namespaces));
    ...
    */
	...
	/**
	 * Returns a string with the output of public_dump
	 *
	 * @param mixed $data The publiciable to public_dump
	 * @return string The output of public_dump
	 * @access public
	 */
    function publicDump($data) {
		ob_start();
		public_dump($data);
...
class nusoap_xmlschema extends nusoap_base  {
	...
	function schemaStartElement($parser, $name, $attrs) {
		...
		switch($name){
			...
			case 'attribute':	// complexType attribute
            	...
            	$this->appendDebug($this->publicDump($attrs));
				if (!isset($attrs['form'])) {
					// TODO: handle globals
	...
	*/
	function schemaEndElement($parser, $name) {
		...
		if($name == 'complexType'){
			...
			$this->xdebug($this->publicDump($this->complexTypes[$this->currentComplexType]));
		...
		if($name == 'simpleType'){
			...
			$this->xdebug($this->publicDump($this->simpleTypes[$this->currentSimpleType]));
		...
		if((! $is_element) && isset($this->complexTypes[$type])){
			...
			if (!isset($this->simpleTypes[$type]['phpType'])) {
				...
				if ($etype) {
					...
					$this->xdebug($this->publicDump($etype));
	...
	* 	'struct',
	* 	'all',
	* 	array('mypublic'=> array('name'=>'mypublic','type'=>'string')
	* );
	* 
	* @param name
	...
	*/
	function addComplexType($name,$typeClass='complexType',$phpType='array',$compositor='',$restrictionBase='',$elements=array(),$attrs=array(),$arrayType=''){
		...
		$this->appendDebug($this->publicDump($this->complexTypes[$name]));
	}
	...
	*/
	function addSimpleType($name, $restrictionBase='', $typeClass='simpleType', $phpType='scalar', $enumeration=array()) {
		...
		$this->appendDebug($this->publicDump($this->simpleTypes[$name]));
	}
	...
	*/
	function addElement($attrs) {
		...
		$this->appendDebug($this->publicDump($this->elements[ $attrs['name'] ]));
	}
}
...
class soap_transport_http extends nusoap_base {
	...
	*/
	function soap_transport_http($url, $curl_options = NULL, $use_curl = false){
		...
		$this->appendDebug($this->publicDump($curl_options));
		$this->setURL($url);
...
//			$this->setError('cURL Extension, or OpenSSL extension w/ PHP version >= 4.3 is required for HTTPS');
		...
		if (ini_get('safe_mode') || ini_get('open_basedir')) {
			...
			$this->appendDebug($this->publicDump(ini_get('safe_mode')));
			$this->debug('open_basedir = ');
			$this->appendDebug($this->publicDump(ini_get('open_basedir')));
		} else {
...
//		}
	...
	*/
	function setCredentials($username, $password, $authtype = 'basic', $digestRequest = array(), $certRequest = array()) {
		...
		$this->appendDebug($this->publicDump($digestRequest));
	...
	*
	* @return	string the response (also sets member publiciables like incoming_payload)
	* @access   private
	*/
...
//		}
		...
		if ($cErr != '') {
			...
			//echo '<pre>';
			//public_dump(curl_getinfo($this->ch));
			//echo '</pre>';
		}
		// close curl
...
class nusoap_server extends nusoap_base {
	...
	*/
	function nusoap_server($wsdl=false){
		...
		if (isset($_SERVER)) {
			...
			$this->appendDebug($this->publicDump($_SERVER));
		} elseif (isset($HTTP_SERVER_publicS)) {
			$this->debug("HTTP_SERVER_publicS is defined:");
			$this->appendDebug($this->publicDump($HTTP_SERVER_publicS));
		} else {
			$this->debug("Neither _SERVER nor HTTP_SERVER_publicS is defined.");
		}
		...
		} elseif (isset($HTTP_SERVER_publicS['QUERY_STRING'])) {
			$qs = explode('&', $HTTP_SERVER_publicS['QUERY_STRING']);
			foreach ($qs as $v) {
				if (substr($v, 0, 6) == 'debug=') {
	...
	*/
	function service($data){
		...
		} elseif (isset($HTTP_SERVER_publicS['REQUEST_METHOD'])) {
			$rm = $HTTP_SERVER_publicS['REQUEST_METHOD'];
		} else {
			$rm = '';
		}
		...
		if (isset($_SERVER['QUERY_STRING'])) {
			...
			$qs = $HTTP_SERVER_publicS['QUERY_STRING'];
		} else {
			$qs = '';
		}
		...
		if ($rm == 'POST') {
			...
			if ($this->externalWSDLURL){
              if (strpos($this->externalWSDLURL, "http://") !== false) { // assume URL
				...
				if ($this->debug_flag) {
					$this->debug('wsdl:');
					$this->appendDebug($this->publicDump($this->wsdl));
	...
	*/
	function parse_http_headers() {
		...
		} elseif (is_array($HTTP_SERVER_publicS)) {
			$this->debug("In parse_http_headers, use HTTP_SERVER_publicS");
			foreach ($HTTP_SERVER_publicS as $k => $v) {
				if (substr($k, 0, 5) == 'HTTP_') {
	...
	function invoke_method() {
		...
		if ($this->wsdl) {
			if ($this->opData = $this->wsdl->getOperationData($this->methodname)) {
				...
				$this->appendDebug('opData=' . $this->publicDump($this->opData));
		...
		$this->appendDebug($this->publicDump($this->methodparams));
        ...
        $this->appendDebug($this->publicDump($this->methodreturn));
	...
	*/
	function serialize_return() {
		...
		$this->appendDebug($this->publicDump($return_val));
	...
	* @return string The HTTP body, which Include the SOAP payload
	* @access private
	*/
	...
	function register($name,$in=array(),$out=array(),$namespace=false,$soapaction=false,$style=false,$use=false,$documentation='',$encodingStyle=''){
		...
		if(false == $soapaction) {
			if (isset($_SERVER)) {
				...
				$HTTPS = isset($_SERVER['HTTPS']) ? $_SERVER['HTTPS'] : (isset($HTTP_SERVER_publicS['HTTPS']) ? $HTTP_SERVER_publicS['HTTPS'] : 'off');
			} elseif (isset($HTTP_SERVER_publicS)) {
				$SERVER_NAME = $HTTP_SERVER_publicS['SERVER_NAME'];
				$SCRIPT_NAME = isset($HTTP_SERVER_publicS['PHP_SELF']) ? $HTTP_SERVER_publicS['PHP_SELF'] : $HTTP_SERVER_publicS['SCRIPT_NAME'];
				$HTTPS = isset($HTTP_SERVER_publicS['HTTPS']) ? $HTTP_SERVER_publicS['HTTPS'] : 'off';
			} else {
				$this->setError("Neither _SERVER nor HTTP_SERVER_publicS is available");
			}
        	if ($HTTPS == '1' || $HTTPS == 'on') {
        		$SCHEME = 'https';
        	} else {
    ...
    */
    function configureWSDL($serviceName,$namespace = false,$endpoint = false,$style='rpc', $transport = 'http://schemas.xmlsoap.org/soap/http', $schemaTargetNamespace = false)
    {
		...
		if (isset($_SERVER)) {
			...
			$SERVER_NAME = $HTTP_SERVER_publicS['SERVER_NAME'];
			$SERVER_PORT = $HTTP_SERVER_publicS['SERVER_PORT'];
			$SCRIPT_NAME = isset($HTTP_SERVER_publicS['PHP_SELF']) ? $HTTP_SERVER_publicS['PHP_SELF'] : $HTTP_SERVER_publicS['SCRIPT_NAME'];
			$HTTPS = isset($HTTP_SERVER_publicS['HTTPS']) ? $HTTP_SERVER_publicS['HTTPS'] : 'off';
		} else {
			$this->setError("Neither _SERVER nor HTTP_SERVER_publicS is available");
...
class wsdl extends nusoap_base {
		...
		foreach($this->ports as $port => $portData) {
			...
			if ($portName == '' || $port == $portName) {
				...
				if ($portData['bindingType'] == $bindingType) {
					...
					//$this->debug("port data: " . $this->publicDump($portData));
					//$this->debug("bindings: " . $this->publicDump($this->bindings[ $portData['binding'] ]));
					// merge bindings
				...
				if ($t) {
					...
					if (!isset($t['phpType'])) {
						...
						if ($etype) {
							...
							$this->debug($this->publicDump($etype));
							if (isset($etype['phpType'])) {
    ...
    */
    function webDescription(){
		...
		if (isset($_SERVER)) {
			...
			$PHP_SELF = $HTTP_SERVER_publicS['PHP_SELF'];
		} else {
			$this->setError("Neither _SERVER nor HTTP_SERVER_publicS is available");
		}
	...
	*/
	...
	function parametersMatchWrapped($type, &$parameters) {
		...
		$this->appendDebug($this->publicDump($parameters));
		...
		$this->appendDebug($this->publicDump($typeDef));
		if (substr($uqType, -1) == '^') {
	...
	function serializeRPCParameters($operation, $direction, $parameters, $bindingType = 'soap') {
		...
		$this->appendDebug('parameters=' . $this->publicDump($parameters));
		...
		$this->appendDebug($this->publicDump($opData));
	...
	function serializeType($name, $type, $value, $use='encoded', $encodingStyle=false, $unqualified=false)
	{
		...
		$this->appendDebug("value=" . $this->publicDump($value));
		if($use == 'encoded' && $encodingStyle) {
		...
		if(!$typeDef = $this->getTypeDef($uqType, $ns)){
			...
			$this->appendDebug('typeDef=' . $this->publicDump($typeDef));
			if (substr($uqType, -1) == '^') {
				$uqType = substr($uqType, 0, -1);
			}
		}
		...
		if ($phpType == 'struct') {
			...
			if (is_object($value)) {
				$value = get_object_publics($value);
			}
			if (is_array($value)) {
...
class nusoap_parser extends nusoap_base {
	...
	function nusoap_parser($xml,$encoding='UTF-8',$method='',$decode_utf8=true){
		...
		if(!empty($xml)){
			...
			if(!xml_parse($this->parser,$xml,true)){
				...
				$this->appendDebug($this->publicDump($this->message));
	...
	*/
	...
	/**
	* decodes simple types into PHP publiciables
	*
	...
	*/
	function buildVal($pos){
		...
		if($this->message[$pos]['children'] != ''){
			...
			$this->appendDebug($this->publicDump($ret));
			return $ret;
		} else {
